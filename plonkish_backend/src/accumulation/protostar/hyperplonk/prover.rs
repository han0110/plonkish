...
where
    ...
    Pcs: PolynomialCommitmentScheme<F, Polynomial = MultilinearPolynomial<F>>,
{
    let [(acc_pow, acc_zeta, acc_zeta_pow_lsqrt, acc_u), (incoming_pow, incoming_zeta, incoming_zeta_pow_lsqrt, incoming_u)] =
        [accumulator, incoming].map(|witness| {
            let pow = witness.witness_polys.last().unwrap();
            let zeta = witness
                .instance
                .challenges
                .iter()
                .nth_back(zeta_nth_back + 1usize)
                .unwrap();
            let zeta_pow_lsqrt = witness
                .instance
                .challenges
                .iter()
                .nth_back(zeta_nth_back)
                .unwrap();
            (pow, zeta, zeta_pow_lsqrt, witness.instance.u)
        });
    assert_eq!(incoming_u, F::ONE);
    ...
    let bh = BooleanHypercube::new(num_vars/2 + 1usize);
    let next_map = bh.rotation_map(Rotation::next());
    parallelize(&mut cross_term, |(cross_term, start)| {
        cross_term
            .iter_mut()
            ...
            .for_each(|(cross_term, b)| {
                if b < l_sqrt {
                    ...
                    *cross_term = acc_pow[next_map[b]] + acc_u * incoming_pow[next_map[b]]
                    - (acc_pow[b] * incoming_zeta_pow_lsqrt + incoming_pow[b] * acc_zeta_pow_lsqrt);
                }
            })
    });

    // todo fix rotations for mid values
    let b_0 = 0;
    let b_lsqrt = bh.rotate(1, Rotation((num_vars/2) as i32));
    let b_lsqrt_next = bh.rotate(b_lsqrt, Rotation::next()); 
    let b_last = bh.rotate(1, Rotation::prev());
    ...
    cross_term[b_lsqrt_next] += acc_pow[b_lsqrt_next] * incoming_zeta_pow_lsqrt 
        + incoming_pow[b_lsqrt_next] * acc_zeta_pow_lsqrt - acc_u.double();

    cross_term[b_last] += acc_pow[b_last] * incoming_zeta_pow_lsqrt 
        + incoming_pow[b_last] * acc_zeta_pow_lsqrt - acc_u * incoming_zeta_pow_lsqrt 
        - acc_zeta_pow_lsqrt;
